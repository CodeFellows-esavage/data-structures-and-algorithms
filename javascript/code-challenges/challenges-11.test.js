...
const totalSum = (input) => {
  // Solution code here...
  // let subSum = [];
  const subSum = input.map(arr => arr.reduce((sum, cur) => {
    sum = sum + cur;
    return sum;
  }, 0));
  return subSum.reduce((sum, cur) => sum = sum + cur);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filteredNum = input.map(arr => arr.filter(num => typeof(num) === 'number' && num % 5 === 0));
  let powerNum = filteredNum.map(arr => { 
    if (arr.length > 0) {
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let people = data.filter(obj => Object.values(obj).includes('male') || Object.values(obj).includes('female'));

  return people.map(person => person.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let heights = Object.values(data).map(charObj => charObj.height);

  let shortest = heights.sort((a, b) => a - b)[0];

  let shortestChar = data.filter(charObj => charObj.height === shortest)[0].name;
  return shortestChar;
};
