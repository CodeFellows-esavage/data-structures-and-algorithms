...
const findHappiness = (arr) => {
  // Solution code here...
  let smileArr = [];
  arr.forEach(str => str.includes(':)') ? smileArr.push(str) : console.log('no happiness'));
  return smileArr;
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < 14; j++) {
      if (j !== 0 && j !== 4 && j !== 5 && j !== 9) {
        numStr = numStr + arr[i].charAt(j);
      }
    }
    phoneNumArr.push(numStr);
  }
  return phoneNumArr;
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 !== 0) oddString = oddString + str.charAt(i);
  }
  return oddString;
};
...
const allHappy = (arr) => {
  ...
  let countTrue = 0;
  arr.forEach(str => {
    if (str.includes(':)')) countTrue = countTrue + 1;
  });
  return arr.length === countTrue ? true : false;
};
